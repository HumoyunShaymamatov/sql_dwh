/*
===========================================================
Procedure: silver.load_silver
Purpose: Transform and cleanse data from bronze layer into
         structured silver tables.

Description:
This procedure performs full refresh transformations
(bronze → silver). It applies:

- Deduplication (ROW_NUMBER for latest customer record)
- Data standardization (gender, marital status, product line)
- Data quality fixes (NULL handling, recalculated sales/price)
- Date validation and conversion
- Basic cleansing (TRIM, REPLACE, formatting)

Each table is truncated before insert to ensure a clean,
deterministic reload.

Intended to be executed after bronze.load_bronze and
before loading the gold layer.
===========================================================
*/

CREATE OR ALTER PROCEDURE silver.load_silver AS
BEGIN
	SET NOCOUNT ON;

	DECLARE	@proc_start DATETIME, @proc_end DATETIME,
			@start_time DATETIME, @end_time DATETIME;

	SET @proc_start = GETDATE();

	BEGIN TRY
		PRINT '===========================================================';
		PRINT 'Starting SILVER Layer Load';
		PRINT '===========================================================';

		/* ================= CRM CUSTOMER ================= */
		PRINT 'Processing crm_cust_info...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.crm_cust_info;

		INSERT INTO silver.crm_cust_info (
			cst_id, cst_key, cst_firstname, cst_lastname,
			cst_gndr, cst_marital_status, cst_create_date
		)
		SELECT
			cst_id,
			cst_key,
			TRIM(cst_firstname),
			TRIM(cst_lastname),
			CASE UPPER(TRIM(cst_gndr))
				WHEN 'F' THEN 'Female'
				WHEN 'M' THEN 'Male'
				ELSE 'n/a'
			END,
			CASE UPPER(TRIM(cst_marital_status))
				WHEN 'S' THEN 'Single'
				WHEN 'M' THEN 'Married'
				ELSE 'n/a'
			END,
			cst_create_date
		FROM (
			SELECT *,
				   ROW_NUMBER() OVER (
					   PARTITION BY cst_id
					   ORDER BY cst_create_date DESC
				   ) AS flag_last
			FROM bronze.crm_cust_info
			WHERE cst_id IS NOT NULL
		) t
		WHERE flag_last = 1;

		SET @end_time = GETDATE();
		PRINT 'crm_cust_info took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';


		/* ================= CRM PRODUCT ================= */
		PRINT 'Processing crm_prd_info...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.crm_prd_info;

		INSERT INTO silver.crm_prd_info (
			prd_id, cat_id, prd_key, prd_nm,
			prd_cost, prd_line, prd_start_dt, prd_end_dt
		)
		SELECT 
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_'),
			SUBSTRING(prd_key, 7, LEN(prd_key)),
			prd_nm,
			COALESCE(prd_cost, 0),
			CASE UPPER(TRIM(prd_line))
				WHEN 'M' THEN 'Mountain'
				WHEN 'R' THEN 'Road'
				WHEN 'S' THEN 'Other Sales'
				WHEN 'T' THEN 'Touring'
				ELSE 'n/a'
			END,
			CAST(prd_start_dt AS DATE),
			CAST(
				LEAD(DATEADD(day, -1, prd_start_dt), 1)
				OVER(PARTITION BY prd_key ORDER BY prd_start_dt)
			AS DATE)
		FROM bronze.crm_prd_info;

		SET @end_time = GETDATE();
		PRINT 'crm_prd_info took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';


		/* ================= CRM SALES ================= */
		PRINT 'Processing crm_sales_details...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.crm_sales_details;

		INSERT INTO silver.crm_sales_details (
			sls_ord_num, sls_prd_key, sls_cust_id,
			sls_order_dt, sls_ship_dt, sls_due_dt,
			sls_sales, sls_quantity, sls_price
		)
		SELECT
			sls_ord_num,
			sls_prd_key,
			sls_cust_id,
			CASE WHEN sls_order_dt = 0 OR LEN(sls_order_dt) != 8
				 THEN NULL
				 ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE)
			END,
			CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8
				 THEN NULL
				 ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
			END,
			CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8
				 THEN NULL
				 ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
			END,
			CASE 
				WHEN sls_sales IS NULL OR sls_sales <= 0
					 OR sls_sales != sls_quantity * ABS(sls_price)
				THEN sls_quantity * ABS(sls_price)
				ELSE sls_sales
			END,
			sls_quantity,
			CASE 
				WHEN sls_price IS NULL OR sls_price <= 0
				THEN ABS(sls_sales) / NULLIF(sls_quantity, 0)
				ELSE sls_price
			END
		FROM bronze.crm_sales_details;

		SET @end_time = GETDATE();
		PRINT 'crm_sales_details took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';


		/* ================= ERP CUSTOMER ================= */
		PRINT 'Processing erp_cust_az12...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.erp_cust_az12;

		INSERT INTO silver.erp_cust_az12 (cid, bdate, gen)
		SELECT
			CASE WHEN cid LIKE 'NAS%' 
				 THEN SUBSTRING(cid, 4, LEN(cid))
				 ELSE cid
			END,
			CASE WHEN bdate > GETDATE()
				 THEN NULL
				 ELSE bdate
			END,
			CASE 
				WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
				WHEN UPPER(TRIM(gen)) IN ('M', 'MALE') THEN 'Male'
				ELSE 'n/a'
			END
		FROM bronze.erp_cust_az12;

		SET @end_time = GETDATE();
		PRINT 'erp_cust_az12 took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';


		/* ================= ERP LOCATION ================= */
		PRINT 'Processing erp_loc_a101...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.erp_loc_a101;

		INSERT INTO silver.erp_loc_a101 (cid, cntry)
		SELECT
			REPLACE(cid, '-', ''),
			CASE 
				WHEN TRIM(cntry) = 'DE' THEN 'Germany'
				WHEN TRIM(cntry) IN ('USA', 'US') THEN 'United States'
				WHEN TRIM(cntry) = '' OR cntry IS NULL THEN 'n/a'
				ELSE TRIM(cntry)
			END
		FROM bronze.erp_loc_a101;

		SET @end_time = GETDATE();
		PRINT 'erp_loc_a101 took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';


		/* ================= ERP PRODUCT CATEGORY ================= */
		PRINT 'Processing erp_px_cat_g1v2...';
		SET @start_time = GETDATE();

		TRUNCATE TABLE silver.erp_px_cat_g1v2;

		INSERT INTO silver.erp_px_cat_g1v2 (id, cat, subcat, maintenance)
		SELECT id, cat, subcat, maintenance
		FROM bronze.erp_px_cat_g1v2;

		SET @end_time = GETDATE();
		PRINT 'erp_px_cat_g1v2 took: ' +
			  CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds.';

		PRINT '===========================================================';
		PRINT 'SILVER Layer Load Completed';
		PRINT '===========================================================';

	END TRY
	BEGIN CATCH
		PRINT '===========================================================';
		PRINT 'ERROR OCCURRED DURING SILVER LAYER LOAD';
		PRINT 'Error Message: ' + ERROR_MESSAGE();
		PRINT 'Error Number: ' + CAST(ERROR_NUMBER() AS NVARCHAR);
		PRINT 'Error State: ' + CAST(ERROR_STATE() AS NVARCHAR);
		PRINT '===========================================================';
	END CATCH

	SET @proc_end = GETDATE();
	PRINT 'Total time: ' +
		  CAST(DATEDIFF(second, @proc_start, @proc_end) AS NVARCHAR) + ' seconds.';
END;
